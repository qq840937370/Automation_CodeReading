...
{
    // 注意: 使用“重构”菜单上的“重命名”命令，可以同时更改代码、svc 和配置文件中的类名“CodeReadingService”。
    // 注意: 为了启动 WCF 测试客户端以测试此服务，请在解决方案资源管理器中选择 CodeReadingService.svc 或 CodeReadingService.svc.cs，然后开始调试。
    //[LoggingServiceBehavior] 
    public class CodeReadingService : ICodeReadingService
    {
        #region 常数
        /// <summary>
        /// 窗体ID
        ...
        public SearchResult Search(SearchConditions searchConditions)
        {
            ...
            using (var cmd = new SqlCommand())
            {
                ...
                using (SqlDataAdapter reader=new SqlDataAdapter(cmd))
                {
                    ...
                    var dt = new CodeReadingDataSet.SearchListDataTable();
                    reader.Fill(dt);
